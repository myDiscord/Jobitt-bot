...
from core.keyboards.smm_keyboards import ikb_keyboard
...
async def channel_poster(bot: Bot, post: Post) -> None:
    current_time = datetime.now()

    data = await post.get_channel_row(current_time)

    if not data:
        await asyncio.sleep(1 * 60)
        await channel_poster(bot, post)
        return

    await sleeper(current_time, data['date_time'])

    asyncio.create_task(post_channel(bot, post, data))

    await channel_poster(bot, post)


async def post_channel(bot: Bot, post: Post, data):
    channel_id = settings.channel.chanel_id
